...
const hdrs = () => ({ Authorization: `Bearer ${safeStorage.get('pos_token', '')}` });
...
export default function KitchenTV() {
  ...
  const [authReady, setAuthReady] = useState(!!safeStorage.get('pos_token'));
  const [searchParams] = useSearchParams();

  // BUG-F2/F3 fix: load via signed kiosk token in URL (`?token=...`).
  // The previous implementation accepted `?pin=XXXX` which leaked the
  // PIN in browser history, server logs, and Referer headers. Tokens are
  // generated by an admin via POST /api/auth/kiosk-token and stored
  // in localStorage on first load.
  useEffect(() => {
    if (authReady) return;
    const t = searchParams.get('token');
    if (t) {
      safeStorage.set('pos_token', t);
      setAuthReady(true);
    }
  ...
  if (!authReady) {
    // BUG-F3 fix: prompt for kiosk token (no longer accept PIN in URL).
      ...
      <div className="min-h-screen bg-black text-white flex items-center justify-center">
        <div className="text-center">
          ...
          {token ? (
            ...
            <div>
              <p className="text-gray-400 mb-4">Solicita a un administrador un token de kiosko (Configuración → Impresión)</p>
              <p className="text-gray-600 text-sm font-mono">Ejemplo: /kitchen-tv?token=eyJhbGc…</p>
            </div>
          )}
        </div>
      </div>
          ...
          <div className="space-y-3">
            ...
            <div className="bg-gray-800 rounded-xl p-4">
              ...
              <div className="bg-black rounded-lg p-3 font-mono text-xs text-green-400 space-y-1">
                ...
                <p className="break-all">"chrome.exe" --kiosk "{window.location.origin}/kitchen-tv?token=TOKEN"</p>
                <p className="text-gray-500 mt-2">Linux:</p>
                <p className="break-all">chromium --kiosk "{window.location.origin}/kitchen-tv?token=TOKEN"</p>
              </div>
              <p className="text-[11px] text-gray-500 mt-1">Genera el TOKEN desde Configuración → Impresión (admin).</p>
            </div>
          </div>
        </DialogContent>
      </Dialog>
