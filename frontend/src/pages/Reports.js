...
export default function Reports() {
  ...
  const renderStockAdjustmentsReport = () => {
    const data = reportData;
    ...
    return (
      <div className="space-y-4" data-testid="stock-adjustments-report">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          ...
          <div className="bg-card border border-border rounded-xl p-4 text-center">
            <div className={`text-2xl font-oswald font-bold ${stats.net_value_impact >= 0 ? 'text-green-400' : 'text-red-400'}`}>
              {formatMoney(stats.net_value_impact)}
            </div>
              ...
              <tbody className="divide-y divide-border">
                {by_ingredient.map((bi, i) => (
                  <tr key={i}>
                    ...
                    <td className={`p-2 text-right font-oswald ${bi.net_quantity >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                      {bi.net_quantity > 0 ? '+' : ''}{bi.net_quantity.toFixed(2)}
                    </td>
        ...
        <div className="bg-card border border-border rounded-xl overflow-hidden">
            ...
            <tbody className="divide-y divide-border">
              {logs.map((log, i) => (
                <tr key={log.id || i} className="hover:bg-muted/30">
                  ...
                  <td className={`p-2 text-right font-oswald font-bold ${log.quantity >= 0 ? 'text-green-400' : 'text-red-400'}`}>
          ...
          {logs.length === 0 && (
            <div className="text-center py-8 text-muted-foreground">No hay ajustes de stock en este período</div>
          )}
        </div>
      </div>
    );
