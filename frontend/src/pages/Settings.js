          ...
          <TabsContent value="system">
            <div className="space-y-6">
              <div>
                ...
                {/* Quick Amounts Configuration */}
                <div className="bg-card border border-border rounded-xl p-4 mb-4">
                  <h3 className="text-sm font-semibold mb-2">Montos Rápidos de Pago</h3>
                  <p className="text-xs text-muted-foreground mb-3">
                    Configura los botones de montos rápidos que aparecen en la pantalla de pago.
                  </p>
                  
                  {/* Current amounts */}
                  <div className="flex flex-wrap gap-2 mb-3">
                    {(systemConfig.quick_amounts || []).map((amount, i) => (
                      <div key={i} className="flex items-center gap-1 bg-primary/20 text-primary rounded-lg px-3 py-1.5">
                        <span className="font-oswald font-bold">{amount.toLocaleString()}</span>
                        <button 
                          onClick={() => setSystemConfig(p => ({
                            ...p, 
                            quick_amounts: p.quick_amounts.filter((_, idx) => idx !== i)
                ...
                <div className="bg-card border border-border rounded-xl p-4 mb-4">
                  <h3 className="text-sm font-semibold mb-2">Métodos de Pago Activos</h3>
                  <p className="text-xs text-muted-foreground mb-3">
                    Los métodos activos aparecerán en la pantalla de pago. Configúralos en la pestaña "Ventas → Pagos".
                  </p>
                  <div className="flex flex-wrap gap-2">
                    {payMethods.filter(p => p.active).map(pm => (
                      <span key={pm.id} className="bg-green-600/20 text-green-500 rounded-lg px-3 py-1.5 text-xs font-semibold">
                        {pm.name}
                      </span>
                    ))}
                    {payMethods.filter(p => !p.active).map(pm => (
                      <span key={pm.id} className="bg-muted text-muted-foreground rounded-lg px-3 py-1.5 text-xs">
                        {pm.name} (inactivo)
                      </span>
                    ))}
                  </div>
