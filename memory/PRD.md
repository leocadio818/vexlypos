...
- **Bottom Sheet Notification System** (DONE): Replaced ALL sonner toasts with custom BottomSheetNotification component. Global `notify` emitter (`/lib/notify.js`). ALL types have X button + auto-dismiss: Error=15s, Warning=5s, Success/Info=3s, Fiscal=10s+progress bar. Max 2 stacked. MutationObserver pauses timers when dialog is open. ~45 files updated.
