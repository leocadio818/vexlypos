...
- **Dashboard Edit Mode UI/UX Bug Fix** (DONE): Fixed two P0 bugs: (1) "Cancelar"/"Guardar" buttons now use inline styles with proper contrast (white/dark backgrounds) against the orange bar. (2) Long-press scroll conflict resolved by: increasing movement tolerance from 8px→15px, attaching a `scroll` event listener on the nearest scrollable ancestor that cancels the timer when native scroll fires, adding `touchAction: 'pan-y'` to allow browser-native vertical scrolling, and increasing long-press duration from 800ms→900ms. Applied to Dashboard.js and CashRegister.js.
